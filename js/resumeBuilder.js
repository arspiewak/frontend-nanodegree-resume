...
var education = {
	...
	display: function() {
		...
		for (var i = 0, len = this.schools.length; i < len; i++){
			...
			for (var j = 0, jlen = this.schools[i].majors.length; j < jlen; j++) {
...
/* Fourth JSON object. Against all reason, I follow the project's requirement
 * that we have an object named "projects" which contains an array also called
 * "projects." Besides risking ambiguous references, this doen't agree with the
 * way every other object is named: singular name for the object, plural names
 * for any arrays in it. I object, but I don't want to have to submit again.
 */
var projects = {
	'projects': [
		{
	...
	display: function() {
		...
		for (var i = 0; i < projLen; i++) {
			...
			for (var j = 0, jlen = thisProj.images.length; j < jlen; j++) {
